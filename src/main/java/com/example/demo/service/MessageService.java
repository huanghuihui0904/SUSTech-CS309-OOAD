...
public class MessageService {
    ...
    public void sendMessage(Integer to, MessageDTO messageDTO) {
        ...
        jdbcTemplate.update("insert into message(messageFromId, messageToId, messageTime, content) " +
                "values (?, ?, ?, ?);", messageDTO.getFromLogin(), to, curTime, messageDTO.getMessage());
    ...
    public List<Map<String, Object>> getListMessageGroups(@PathVariable("groupid") Integer groupid) {
        return jdbcTemplate.queryForList(
                "select gm.*, c.name as name\n" +
                        ...
                        "where gm.groupId = ?\n" +
                        "order by groupMessageId;", groupid);
    }
